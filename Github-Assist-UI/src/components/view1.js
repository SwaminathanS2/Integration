...
export function FeatureFileViewer({
  ...
  // Emit width changes while open; cleanup emits closed
  useEffect(() => {
    if (!open) return;
...
/**
 * viewRawFile: fetches raw file content from the backend.
 * IMPORTANT: Pass the feature branch (feature switch name) as `branch` from the caller.
 */
export async function viewRawFile(
  ...
  const body = {
    ...
    FeatureName: "",
    Token: "",
    Branch: branch || "", // <-- Home.js will pass the feature switch name here
  };
